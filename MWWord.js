...
/**
 * Eventhandler when a word gets dragged - stores some current positions that
 * later may have to be restored
 * 
 * @param event
 */
...
/**
 * Eventhandler when a word is dragged - some offsets are calculated, all other
 * words get new alpha values
 */
function DragMoveFunction(event)
{
	...
	word.BoundingBox.setOffset(word.pDrawnPointsOffset.x,
			word.pDrawnPointsOffset.y);
...
function DragEndFunction(event)
{
	...
	{
		// Debugger.log(word.sWord+":UPDATE DRAWN POINT ARRAY because
		// originalrotation != textrotation");
	...
	for ( var i = 0; i < layerChildren.length; i++)
	{
		var comparingWord = window.TextHandler.GetWord(layerChildren[i]
				.getName());
	...
	{
		// Debugger.log(word.sWord+": NO WORDS TO COMPARE BECAUSE OF THE
		// BOUNDING BOX");
		window.textlayer.add(word.textShape);
	...
	{
		// Debugger.log(word.sWord+": "+aWordsToCompare.length+" WORDS TO
		// COMPARE BECAUSE OF THE BOUNDING BOX");
	}

	/*
	 * collision detection - when a bigger word is under the current drop
	 * position, current word is moved back again - when no bigger word is under
	 * the current drop position, all smaller words have to move, current word
	 * is dropped
	 */
	...
	for ( var i = 0; i < word.aDrawnPoints.length; i++)
	{
		if (!bReturnToOriginalPosition)
		{
			...
			for ( var j = 0; j < aWordsToCompare.length; j++)
			{
				...
				if (!aWordsAlreadyToMove[j]
						&& aWordsToCompare[j].intersects(x, y))
				{
					var comparingWord = window.TextHandler
							.GetWord(aWordsToCompare[j].getName());
					if (comparingWord.bPinned
							|| aWordsToCompare[j].getFontSize() > word.textShape
									.getFontSize())
					{
						Debugger.log("COLLISION of " + word.sWord
								+ "(smaller) with "
								+ aWordsToCompare[j].getName() + "(bigger)");
						...
						word.textShape
								.transitionTo({
									...
									y : word.pStart.y,
									duration : 0.2,
									rotation : (word.iOriginalRotation)
											* Math.PI / 180,
									callback : function()
									{
										// Debugger.log("CALLBACK of transition
										// of "+word.sWord);
										word.pPos.x = word.pStart.x;
										word.pPos.y = word.pStart.y;
										word.pDrawnPointsOffset.x = word.pPos.x
												- word.pDrawnPointsPosition.x;
										word.pDrawnPointsOffset.y = word.pPos.y
												- word.pDrawnPointsPosition.y;
										word.textShape.saveData();
										word.textShape
												.setRotationDeg(word.iOriginalRotation);
										...
										word.BoundingBox.setOffset(
												word.pDrawnPointsOffset.x,
												word.pDrawnPointsOffset.y);
										// word.BoundingBox.Draw();
					...
					else
					{
						Debugger.log("COLLISION of " + word.sWord
								+ "(bigger) with "
								+ aWordsToCompare[j].getName() + "(smaller)");
						// add smaller word to the ones that have to move -
						// moving is done later

						aWordsAlreadyToMove[j] = true;
						aWordsToMove.push(window.TextHandler
								.GetWord(aWordsToCompare[j].getText()));
					}
...
function MWWord(word)
{
	...
	this.CreateDrawnPointArray = function()
	{
		...
		for ( var y = this.pPos.y - this.iSpanWidth; y < this.pPos.y
				+ this.iSpanWidth; y += 2)
		{
			for ( var x = this.pPos.x - this.iSpanWidth; x < this.pPos.x
					+ this.iSpanWidth; x += 2)
			{
				overall_comparisons++;
		...
		// Debugger.log(this.sWord+": DRAWNPOINTS: length:
		// "+this.aDrawnPoints.length);
		this.pDrawnPointsPosition = new POINT(this.pPos.x, this.pPos.y);
		this.pDrawnPointsOffset = new POINT(0, 0);
	};

	/**
	 * Select this word, this function is called when this word gets selected
	 * with a mouseclick
	 */
	this.Select = function()
	{
		...
		// Set interface for changing this word visible - set all options to the
		// values of this word
		...
		document.getElementById('textFillColor_page2').color
				.fromString(this.sFillColor.substring(1, 7));
		document.getElementById('borderColor_page2').color
				.fromString(this.sBordercolor.substring(1, 7));

		if (!this.bSelected)
		{
			// add the selection shapes to the selectionlayer so that they are
			// visible
	...
	/**
	 * Updates the selection shape of this word - position, width, height, color
	 * and rotation Creates a Rectangle and a Rotationpoint and a line
	 * connecting them
	 */
	this.UpdateSelectionShape = function()
	{
		...
		var selectionShapeRect = new Kinetic.Rect({
			...
			strokeWidth : 1,
			centerOffset : [ this.textShape.getTextWidth() / 2,
					this.textShape.getTextHeight() / 2 ], // for rotation
			draggable : true
		});
		this.selectionShapeRect = selectionShapeRect;

		// add eventlistener to the selectionshaperect - the same as when
		// dragging a word
		...
		var selectionShapeLine = new Kinetic.Line({
			name : this.sWord,
			points : [
					this.pPos.x + this.textShape.getTextHeight() / 2
							* Math.sin(this.iTextRotation * Math.PI / 180),
					this.pPos.y - this.textShape.getTextHeight() / 2
							* Math.cos(this.iTextRotation * Math.PI / 180),
					this.pPos.x + this.textShape.getTextHeight() / 2
							* Math.sin(this.iTextRotation * Math.PI / 180) + 20
							* Math.sin(this.iTextRotation * Math.PI / 180),
					this.pPos.y - this.textShape.getTextHeight() / 2
							* Math.cos(this.iTextRotation * Math.PI / 180) - 20
							* Math.cos(this.iTextRotation * Math.PI / 180) ],
			stroke : window.Canvas.sSelectionColor,
			strokeWidth : 1,
			lineCap : "round",
		...
		var selectionShapeRotationPoint = new Kinetic.Circle({
			name : this.sWord,
			x : this.pPos.x + this.textShape.getTextHeight() / 2
					* Math.sin(this.iTextRotation * Math.PI / 180) + 20
					* Math.sin(this.iTextRotation * Math.PI / 180),
		...
		// rotate the selection shape rect - the rotation of the other two are
		...
		this.selectionShapeRotationPoint.on("dragmove", function(event)
		{
			...
			var vec1X = word.textShape.getTextHeight() / 2
					* Math.sin(word.iTextRotation * Math.PI / 180) + 20
					* Math.sin(word.iTextRotation * Math.PI / 180);
			var vec1Y = -word.textShape.getTextHeight() / 2
					* Math.cos(word.iTextRotation * Math.PI / 180) - 20
					* Math.cos(word.iTextRotation * Math.PI / 180);
			var vec2X = event.shape.getX() - word.pPos.x;
			var vec2Y = event.shape.getY() - word.pPos.y;

			// calculate the new rotation angle
			var cosAngle = (vec1X * vec2X + vec1Y * vec2Y)
					/ (Math.sqrt(vec1X * vec1X + vec1Y * vec1Y) * (Math
	...
	/**
	 * Searches for a new position for the current word - because it is moved
	 * when a bigger word gets dropped at its position - or at the creation of
	 * the word cloud
	 */
	this.MoveToNewPosition = function(beginning)
	{
		...
		// only one of these can be true, it is the direction that the word has
		// to move when a collision occurs
		var bDown = false;
		var bUp = false;
		var bRight = true;
		var bLeft = false;
		...
		var iCount = 0;
		// how often the word has to be moved in one direction, before the
		...
		// remove this shape to get only the children of the layer that need to
		// be compared
		...
		{
			// Debugger.log(this.sWord+"("+pCurrentPos.x+","+pCurrentPos.y+") :
			// NO WORDS TO COMPARE BECAUSE CURRENTLY THERE EXISTS ONLY THIS
			// WORD");
		...
		while (!bCanBeDrawn)
		{
			...
			// get an array of the words that need to be compared because of the
			// collision of their bounding boxes
			var aWordsToCompare = new Array();
			for ( var i = 0; i < layerChildren.length; i++)
			{
				overall_comparisons++;
			...
			{
				// Debugger.log(this.sWord+"("+pCurrentPos.x+","+pCurrentPos.y+")
				// : NO WORDS TO COMPARE BECAUSE OF THE BOUNDING BOX");
				// window.textlayer.add(this.textShape);
				// this.textShape.saveData();
				break;
			}
			...
			{
				// Debugger.log(this.sWord+"("+pCurrentPos.x+","+pCurrentPos.y+")
				// : "+ aWordsToCompare.length+" WORDS TO COMPARE BECAUSE OF THE
				// BOUNDING BOX");
			...
			// go through all the pixels of the current text and look if it
			// collides with a pixel at the layer
			for ( var i = 0; i < this.aDrawnPoints.length; i++)
			{
				...
				for ( var j = 0; j < aWordsToCompare.length; j++)
				{
					overall_comparisons++;
					comparisons++;
					// if collision is detected, move the current text by a
					// certain amount
		...
		// move the shape back to the startposition and make a nice transition
		// to the new position
		if (!beginning)
		{
	...
	/**
	 * Draws this word Only called at creation of the word cloud
	 */
	this.Draw = function(stageWidth, stageHeight, aDrawnWords)
	{
		...
		// add text to the layer (for being able to get parameters as width,
		// height or intersects)
		window.textlayer.add(this.textShape);
		this.textShape.saveData();

		// get the maximum span width of the word
		this.iSpanWidth = Math.max(this.textShape.getTextWidth(),
				this.textShape.getTextHeight());
		...
		this.textShape.on("mousedown",
				function(event)
				{
					var currentWord = window.TextHandler.GetWord(event.shape
							.getName());
	...
	/**
	 * Updates the drawing of the word - no change in position, only in its
	 * parameters like color etc.
	 */
	this.UpdateDrawing = function(updatedrawnpointsarray)
	{
		...
		// create the drawn point array again if the shape of the text is
		// changed
